...
const Map = () => {
  ...
  const reverseCoordinates = (arr) => {
    if (arr && arr.length >= 2) { // Check if array exists and has at least 2 elements
      return [arr[1], arr[0]];
    } else {
      return null;
    }
  ...
  useEffect(() => {
    ...
    if (selectedCoordinates && Array.isArray(selectedCoordinates)) { // Check if selectedCoordinates is an array
